...
Свой флот — не угроза, и после одной вставки он весь вечер шумит в каждом
локале. Кнопка с перечёркнутым кругом в шапке открывает меню: игнорировать
выделенных, игнорировать всех из последней проверки или очистить список.
Правый клик по пилоту в списке делает то же самое для него одного.

Игнорируемый пилот отбрасывается **до** любого поиска, поэтому не стоит ни
чтения кэша, ни запроса к киллборде — флот из сорока человек становится
бесплатным, а не проверенным и выброшенным.

Список никогда не пишется на диск. Он переживает закрытие окна (оно только
прячется в трей) и исчезает вместе с приложением.
...
Второй запуск не поднимает вторую копию: уже запущенная показывает своё окно,
а новый процесс завершается. Дело не в аккуратности — две копии слали бы вдвое
больше запросов с одного адреса, а zKillboard банит по адресу.
...
публичного репозитория и сборки в CI — теперь есть и то и другое, так что это
может измениться. Самоподписанный сертификат
ставить не буду — Windows всё равно скажет «Неизвестный издатель», и он
создавал бы ложное впечатление проверенности.

Что можно сделать вместо того, чтобы верить на слово:

* собрать из исходников самому — вся программа здесь, и `python build.py`
  даёт ровно то же самое;
* взять сборку из [релизов](https://github.com/kersidjay69-art/Character-Check/releases):
  её собирает GitHub Actions из того самого коммита, на чужой машине, и
  прикладывает к ней провенанс. Проверяется командой
  `gh attestation verify CharacterCheck-win64.zip -R kersidjay69-art/Character-Check`.
  Это не подпись кода — Windows всё равно скажет «неизвестный издатель», — но
  это проверяемое утверждение о том, откуда взялся файл.
...
поднимется само. Молча: ни всплывающего уведомления, ни звука. О находке
говорит само окно и иконка в трее, которая красится в цвет худшего вердикта.
...
В шапке две квадратные кнопки: круглая стрелка проверяет то, что сейчас в
буфере, корзина очищает список. У всего в окне есть всплывающая подсказка, а
нижняя строка — это контакты автора.

**Интерфейс только на английском.** До версии 0.1 он был на двух языках,
русскую половину убрали. Этот перевод документации остаётся — он не часть
интерфейса.
...
`window_opacity` — ползунок в правом нижнем углу окна, в процентах.
Ограничен снизу пятьюдесятью и при перетаскивании, и при чтении файла:
прозрачность действует и на текст, а ниже примерно половины имена пилотов
перестают читаться.

Если вы обновились с 0.1, в вашем файле остались `"lang"` и `"sound"`. Оба
больше ни на что не влияют, и никто их за вас не удаляет — файл ваш.
